...
- **Help docs**: The in-app Help page (`/help`) renders `docs/admin-guide.{locale}.md` (`zh-CN`/`en`, falls back to `en`) — this is the operator-facing manual, distinct from the internal `docs/requirements.md`. Update it whenever a change is user-visible: a new or changed device/node **protocol**, a config field on the add/edit forms, a new page, action, or button, the meaning of a status, a system setting, the config/subscription a device exports, or operator-facing behavior (cert modes, certd/webhooks, upgrade/uninstall flow, etc.). Edit **both** `admin-guide.zh-CN.md` and `admin-guide.en.md` in the same change and keep them in sync (same sections/structure). Skip purely internal changes — refactors, Agent protocol wiring, DB migrations — which have no user-visible surface.
