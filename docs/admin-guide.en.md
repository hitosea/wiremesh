...
On the node detail page, **Certificate Status** is collapsed by default. Expand it to see whether the current certificate is valid, its expiry time, days remaining, issuer, renewal method, and renewal window. The status is generated by parsing the certificate already stored by the platform; no separate status record is stored.
