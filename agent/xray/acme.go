...
func AutoCert(cfg *api.XrayConfig, client *api.Client) error {
	...
	for _, inb := range cfg.Inbounds {
		...
		if processedDomains[inb.TlsDomain] {
			continue
		}
		processedDomains[inb.TlsDomain] = true

		domain := inb.TlsDomain
		certPath := fmt.Sprintf("%s/%s.crt", XrayConfigDir, domain)
		...
		// Check if port 80 failed to bind — this is a node-level error; abort all domains
		select {
		case err := <-errCh:
		...
		if err != nil {
			log.Printf("[acme] Certificate request failed for %s: %v (skipping)", domain, err)
			continue
		}
		...
		if err != nil {
			log.Printf("[acme] Failed to marshal private key for %s: %v (skipping)", domain, err)
			continue
		}
		keyPEM := pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: privDER})

		if err := os.WriteFile(certPath, certPEM, 0644); err != nil {
			log.Printf("[acme] Failed to write cert for %s: %v (skipping)", domain, err)
			continue
		}
		if err := os.WriteFile(keyPath, keyPEM, 0600); err != nil {
			log.Printf("[acme] Failed to write key for %s: %v (skipping)", domain, err)
			continue
		}
