...
```bash
# 下载 docker-compose.yml，然后创建 .env 填入必需变量
cat > .env <<EOF
...
cd wiremesh
cp .env.example .env
# 编辑 .env：设置 PUBLIC_URL，并填入 JWT_SECRET / ENCRYPTION_KEY
#   JWT_SECRET=$(openssl rand -base64 48)
#   ENCRYPTION_KEY=$(openssl rand -hex 32)
docker compose up -d --build
```

> **重要：** `JWT_SECRET` 和 `ENCRYPTION_KEY` 是必填项，缺失时容器会拒绝启动。务必妥善保管 `ENCRYPTION_KEY`：一旦丢失，所有已加密字段（节点私钥、SOCKS5 密码等）都将无法解密。
...
| `JWT_SECRET` | JWT 签名密钥（至少 32 字符）。生成：`openssl rand -base64 48` | **必填** |
| `ENCRYPTION_KEY` | AES-256-GCM 密钥（64 位十六进制字符串）。生成：`openssl rand -hex 32` | **必填** |
